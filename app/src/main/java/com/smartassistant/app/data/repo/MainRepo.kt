...
import com.smartassistant.app.backup.BackupManager
...
class MainRepo(private val ctx: Context) {
    ...
    suspend fun allCustomers() = db.customerDao().allSync()
    ...
    suspend fun addNote(cid: Long, text: String) {
        db.noteDao().insert(CustomerNote(customerId = cid, text = text))
        log("إضافة ملاحظة", "عميل #$cid")
    }

    // ===== الأصناف =====
    fun productsPaged(q: String?, tab: String?) = when {
        !q.isNullOrBlank() -> db.productDao().searchPaged(q)
        !tab.isNullOrBlank() && tab != "ALL" -> db.productDao().tabPaged(tab, Fmt.today())
        else -> db.productDao().allPaged()
    }
    val lowStock = db.productDao().lowStockList()
    ...
    suspend fun inventoryOnce(pid: Long) = db.inventoryDao().byProduct(pid)
    fun inventoryFlow(pid: Long) = db.inventoryDao().watch(pid)
    ...
    fun dues(tab: String) = when (tab) {
        ...
        "WEEK" -> db.dueDao().week(Fmt.today(), Fmt.plusDays(7))
        "OVERDUE" -> db.dueDao().overdue(Fmt.today())
        else -> db.dueDao().all()
    }
    val forgotten = db.dueDao().forgotten(Fmt.today())
    val upcoming = db.dueDao().upcomingFlow(Fmt.today())
    ...
    suspend fun markFollowed(id: Long) {
        db.dueDao().markFollowed(id)
        log("تمت المتابعة", "موعد #$id (ليس سداداً)")
    }
    ...
    val allActivity = db.logDao().all()

    // ===== النسخ الاحتياطي =====
    val backups = db.backupDao().all()
    suspend fun createBackup(auto: Boolean): Backup? {
        val b = BackupManager.create(ctx, auto)
        if (b != null) { db.backupDao().insert(b); log("نسخ احتياطي", if (auto) "تلقائي" else "يدوي") }
        return b
    }
    suspend fun restoreBackup(path: String): Boolean {
        val ok = BackupManager.restore(ctx, File(path))
        if (ok) log("استعادة نسخة", path)
        return ok
    }
